...
public class ClientImpl implements Client {
    ...
    private boolean uniqueConnectionPerThread = false;
    ...
    private ThreadLocal<ProtocolHandler> tlProtocolHandler = new ThreadLocal<ProtocolHandler>() {
        ...
        protected ProtocolHandler initialValue() {
            return new ProtocolHandler(host, port, DEFAULT_TIMEOUT);
        }
    };
    ...
    public ClientImpl(String host, Integer port, Integer timeout) {
        ...
        aProtocolHandler = new ProtocolHandler(host, port, timeout);
    }
    ...
    public long put(long priority, int delaySeconds, int timeToRun, byte[] data) {
        ...
        if (response != null && response.getStatus().equals("JOB_TOO_BIG")) {
            BeanstalkException be = new BeanstalkException(BeanstalkExceptionType.JOBTOOBIG, response.getStatus());
            throw be;
        }
    ...
    public Job reserve(Integer timeoutSeconds) {
        ...
        if (response != null && response.getStatus().equals("DEADLINE_SOON")) {
            BeanstalkException be = new BeanstalkException(BeanstalkExceptionType.DEADLINESOON, response.getStatus());
            throw be;
        }
        if (response != null && response.isMatchOk()) {
